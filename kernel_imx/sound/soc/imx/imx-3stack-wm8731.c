/*
 * imx-3stack-wm8731.c  --  i.MX 3Stack Driver for Freescale WM8731 Codec
 *
				      ...
				      struct snd_pcm_hw_params *params)
{
	...
	snd_soc_dai_set_sysclk(codec_dai, WM8731_SYSCLK, 11289600, SND_SOC_CLOCK_IN);
...
/*
 * imx_3stack WM8731 audio DAI opserations.
 */
...
static void imx_3stack_init_dam(int ssi_port, int dai_port)
{
	...
	/* WM8731 uses SSI2 via AUDMUX port dai_port for audio */
...
static int imx_3stack_wm8731_init(struct snd_soc_codec *codec)
{
...
static int __devinit imx_3stack_wm8731_probe(struct platform_device *pdev)
{
	struct mxc_audio_platform_data *plat = pdev->dev.platform_data;
	struct snd_soc_dai *wm8731_cpu_dai = NULL;

	gpio_activate_audio_ports();
	imx_3stack_init_dam(plat->src_port, plat->ext_port);

	if (plat->src_port == 2)
		wm8731_cpu_dai = imx_ssi_dai[2];
	else if (plat->src_port == 1)
		wm8731_cpu_dai = imx_ssi_dai[0];
	else if (plat->src_port == 7)
		wm8731_cpu_dai = imx_ssi_dai[4];

	imx_3stack_dai.cpu_dai = wm8731_cpu_dai;
	return 0;
}

static int imx_3stack_wm8731_remove(struct platform_device *pdev)
{
	return 0;
}

static struct platform_driver imx_3stack_wm8731_audio_driver = {
	.probe = imx_3stack_wm8731_probe,
	.remove = imx_3stack_wm8731_remove,
	.driver = {
...
static int __init imx_3stack_init(void)
{
	...
	ret = platform_driver_register(&imx_3stack_wm8731_audio_driver);
	if (ret)
		return -ENOMEM;
...
MODULE_DESCRIPTION("WM8731 Driver for i.MX 3STACK");
MODULE_LICENSE("GPL");
