				 ...
				 enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_STANDBY:
		if (codec->bias_level == SND_SOC_BIAS_OFF) {
			...
			for (i = 0; i < ARRAY_SIZE(wm8731_reg); i++) {
				if (i != WM8731_ACTIVE && i != WM8731_PWR && cache[i] == wm8731_reg[i])
					continue;
	...
	case SND_SOC_BIAS_OFF:
		data[0] = (WM8731_ACTIVE << 1);
		data[1] = 0;
		codec->hw_write(codec->control_data, data, 2);
		data[0] = (WM8731_PWR << 1) | (0x1);
		data[1] = 0xFF;
		codec->hw_write(codec->control_data, data, 2);
		break;
	}
			   ...
			   enum snd_soc_control_type control)
{
	...
	wm8731->reg = regulator_get(NULL, "VAUDIO");
	if (!wm8731->reg)
	{
	...
	{
		regulator_enable(wm8731->reg);
		regulator_set_voltage(wm8731->reg, 0, 3000000);
	}
...
err_regulator_enable:
	regulator_disable(wm8731->reg);
	regulator_put(wm8731->reg);
	wm8731->reg = NULL;
err:
	kfree(wm8731);
	return ret;
