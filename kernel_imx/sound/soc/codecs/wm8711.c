	...
	struct snd_soc_dai *dai)
{
	...
	u16 iface = snd_soc_read(codec, WM8711_IFACE);
	...
	enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_OFF:
		data[0] = (WM8711_ACTIVE << 1);
		data[1] = 0;
		codec->hw_write(codec->control_data, data, 2);
		data[0] = (WM8711_PWR << 1) | (0x1);
		data[1] = 0xFF;
		codec->hw_write(codec->control_data, data, 2);
		break;
	}
...
static int wm8711_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	struct wm8711_priv *wm8711 = codec->drvdata;

	wm8711_set_bias_level(codec, SND_SOC_BIAS_OFF);

	/* Disable audio_vcc */
	if (wm8711->reg)
		regulator_disable(wm8711->reg);
	return 0;
}
...
static int wm8711_probe(struct platform_device *pdev)
{
	...
	snd_soc_write(codec, WM8711_APANA, 0x18);
	return ret;
			   ...
			   enum snd_soc_control_type control)
{
	...
	wm8711->reg = regulator_get(NULL, "VAUDIO");
	if (!wm8711->reg)
	{
	...
	{
		regulator_enable(wm8711->reg);
		regulator_set_voltage(wm8711->reg, 0, 3000000);
	}
