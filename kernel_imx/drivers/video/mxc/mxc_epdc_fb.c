...
extern void enable_pwrcom(int enable);
extern int set_vcom_voltage(void);
extern int enable_vdd(int enable);

atomic_t screensplash_flag = ATOMIC_INIT(0);
...
static void epdc_powerup(struct mxc_epdc_fb_data *fb_data)
{
	...
	/*Set a screen splash flag*/
	atomic_set(&screensplash_flag, 1);
...
static void epdc_powerdown(struct mxc_epdc_fb_data *fb_data)
{
	...
	atomic_set(&screensplash_flag, 0);
	mutex_unlock(&fb_data->power_mutex);
}
				   ...
				   struct fb_info *info)
{
	...
	if ((fb_data->waiting_for_idle) ||
		...
		spin_unlock_irqrestore(&fb_data->queue_lock, flags);
#if 1
        if(fb_data->blank != FB_BLANK_UNBLANK){
            fb_data->blank = FB_BLANK_UNBLANK;
            return 0;
        }
#endif
...
static ssize_t
waveform_show(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static struct device_attribute waveform =
	__ATTR(waveform, S_IRUGO, waveform_show, NULL);
...
static int mxc_epdc_fb_init_hw(struct fb_info *info)
{
	...
	wv_file = ioremap(WAVEFORM_BASE, WAVEFORM_SIZE);
	...
	wv_data_offs = ALIGN(sizeof(wv_file->wdh) + fb_data->trt_entries + 1, 4);
	file_size = *(int *)((u32)wv_file + wv_data_offs);
	strcpy(fb_data->fw_str, (const char *)wv_file + wv_data_offs + 8);
	wv_data_offs = *(int *)((u32)wv_file + wv_data_offs + 4);
	fb_data->waveform_buffer_size = file_size - wv_data_offs;
	...
	iounmap(wv_file);
	free_bootmem(WAVEFORM_BASE, WAVEFORM_SIZE);
	...
	device_create_file(fb_data->info.dev, &waveform);
...
int __devinit mxc_epdc_fb_probe(struct platform_device *pdev)
{
	...
	struct mxcfb_update_data update;
#endif
	printk("Start probe the mxc epdc fb device\n");
