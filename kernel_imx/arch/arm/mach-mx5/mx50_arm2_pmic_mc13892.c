...
#define REG_MODE_0_ALL_MASK	(DIG_STBY_MASK \
				| USB2_STBY_MASK | PLL_STBY_MASK | IOHI_STBY_MASK)
...
#define	SWMODE_AUTO	0xC

#define THSEL_BIT_LSH		8
#define THSEL_BIT_WID		1
#define GEN1_STBY_LSH		1
#define GEN1_STBY_WID		1
#define GEN2_MODE_MASK_LSH		14
#define GEN2_MODE_MASK_WID		1
...
void vsd1_standby_enable(int enable)
{
	unsigned int value, register_mask;

	value = BITFVAL(GEN1_STBY, enable);
	register_mask = BITFMASK(GEN1_STBY);
	pmic_write_reg(REG_MODE_0, value, register_mask);

}
void vgen2_lp_mode_enable(int enable)
{
	unsigned int value, register_mask;

	value = BITFVAL(GEN2_MODE_MASK, enable);
	register_mask = BITFMASK(GEN2_MODE_MASK);
	pmic_write_reg(REG_MODE_0, value, register_mask);

}

EXPORT_SYMBOL(vgen2_lp_mode_enable);
EXPORT_SYMBOL(vsd1_standby_enable);
...
static int mc13892_regulator_init(struct mc13892 *mc13892)
{
	...
	value = BITFVAL(COINCHEN, 1) | BITFVAL(VCOIN, VCOIN_3_0V) | BITFVAL(THSEL_BIT, 1);
	register_mask = BITFMASK(COINCHEN) | BITFMASK(VCOIN) | BITFMASK(THSEL_BIT);
