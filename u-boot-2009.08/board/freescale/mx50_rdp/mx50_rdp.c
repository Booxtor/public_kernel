...
int setup_splash_img(void)
{
	...
	n = mmc->block_dev.block_read(mmc_dev, blk_start,
					blk_cnt, (u_char *)lcd_base);

	return (n == blk_cnt) ? 0 : -1;
#endif
}
#endif
#ifdef CONFIG_ELINK_97_PANEL
vidinfo_t panel_info = {
	...
	.vl_flag = 0,
	.vl_bpix = 3,
	.vscan_holdoff = 8,		/* vscan_holdoff */
	.sdoed_width   = 10,	/* sdoed_width */
	.sdoed_delay   = 20,	/* sdoed_delay */
	.sdoez_width   = 10,	/* sdoez_width */
	.sdoez_delay   = 20,	/* sdoez_delay */
	.gdclk_hp_offs = 632,	/* gdclk_hp_offs */
	.gdsp_offs     = 20,	/* gdsp_offs */
	.gdoe_offs     = 0,		/* gdoe_offs */
	.gdclk_offs    = 1,		/* gdclk_offs */
	.num_ce        = 3,		/* num_ce */
	cmap:0,
};
#endif
#ifdef  CONFIG_ELINK_60_PANEL
vidinfo_t panel_info = {
	.vl_refresh = 85,
	.vl_col = 800,
	.vl_row = 600,
	.vl_pixclock = 32010660,
	.vl_left_margin = 8,
	.vl_right_margin = 166,
	.vl_upper_margin = 4,
	.vl_lower_margin = 26,
	...
	.vl_bpix = 3,
	cmap:0,
	.vscan_holdoff = 4,		/* vscan_holdoff */
	.sdoed_width   = 10,	/* sdoed_width */
	.sdoed_delay   = 20,	/* sdoed_delay */
	.sdoez_width   = 10,	/* sdoez_width */
	.sdoez_delay   = 20,	/* sdoez_delay */
	.gdclk_hp_offs = 428,	/* gdclk_hp_offs */
	.gdsp_offs     = 20,	/* gdsp_offs */
	.gdoe_offs     = 0,		/* gdoe_offs */
	.gdclk_offs    = 1,		/* gdclk_offs */
	.num_ce        = 1,		/* num_ce */
...
#ifdef  CONFIG_ELINK_60_HD_PVI_PANEL
vidinfo_t panel_info = {
	.vl_refresh = 85,
	.vl_col = 1024,
	.vl_row = 758,
	.vl_pixclock = 36910060,
	.vl_left_margin = 8,
	.vl_right_margin = 32,
	.vl_upper_margin = 4,
	.vl_lower_margin = 8,
	...
	.sdoez_width   = 10,	/* sdoez_width */
	.sdoez_delay   = 20,	/* sdoez_delay */
	.gdclk_hp_offs = 520,	/* gdclk_hp_offs */
	.gdsp_offs     = 20,	/* gdsp_offs */
	.gdoe_offs     = 0,		/* gdoe_offs */
	.gdclk_offs    = 1,		/* gdclk_offs */
	.num_ce        = 1,		/* num_ce */
...
#ifdef  CONFIG_ELINK_60_HD_PANEL
vidinfo_t panel_info = {
	.vl_refresh = 85,
	.vl_col = 1024,
	.vl_row = 768,
...
void epdc_power_on(void)
{
...
#ifdef CONFIG_IMX_CSPI
	/* The VGEN1 must be enabled before PWRCOM */
	setup_power();
#endif
	...
	if (i2c_probe(0x48) == 0)
	{
		byte = 0x90;	/* Set vcom voltage to 1.562mV */
		i2c_write(0x48, 0x04, 1, &byte, 1);
		byte = 0xA3;    /* Use vcom in i2c register */
		i2c_write(0x48, 0x03, 1, &byte, 1);
		byte = 0xBF;    /* Enable vdd */
		i2c_write(0x48, 0x01, 1, &byte, 1);
	}
	else
	{
		printf("i2c_probe(0x48) failed.\n");
	}
}
...
static int mc13892_adc_convert(struct spi_slave  *slave, int channel)
{
	unsigned int val;
	unsigned int result;
	unsigned int adc_0_reg, adc_1_reg;
	unsigned int channel_0 = channel;
	unsigned int channel_1 = 0;

	adc_0_reg = 0;
	adc_0_reg = 0x0 & 0x00003F;
	adc_0_reg |= ADC_INC |ADC_CHRGRAW_D5;
	pmic_reg(slave, REG_ADC0, adc_0_reg, 1);

	adc_1_reg = 0;
	adc_1_reg |= ADC_SGL_CH;
	adc_1_reg |= (channel_0<<ADC_CH_0_POS) & ADC_CH_0_MASK;
	adc_1_reg |= (channel_1<<ADC_CH_1_POS) & ADC_CH_1_MASK;
	adc_1_reg |= ADC_NO_ADTRIG | ADC_EN;
	adc_1_reg |= ASC_ADC;
	pmic_reg(slave, REG_ADC1, adc_1_reg, 1);

	while(!((val = pmic_reg(slave, 0, 0, 0)) & 0x01))
	{
		udelay(20);
	}

	val = pmic_reg(slave, REG_ADC2, 0, 0);
	result = (val & ADD1_RESULT_MASK)>>2;
...
#endif
	if(__REG(SRC_BASE_ADDR + 0x8) == 0x0011)
	{
		goto out1;
	}
...
out1:
	...
	/*Open the power LED5 light*/
	mxc_request_iomux(MX50_PIN_EPITO, IOMUX_CONFIG_ALT1);
	val = readl(GPIO6_BASE_ADDR + 0x04);
	val |= (1<<27);
	writel(val, GPIO6_BASE_ADDR + 0x04);

	val = mc13892_adc_convert(slave, 0);
	val = (val*4692)/1000;
	if(val <= 3370)
	{
		/*Control the LED */
		...
		for (i=0; i<3; i++) {
			val = readl(GPIO6_BASE_ADDR + 0x0);
			val |=  (1 << 27);
			writel(val, GPIO6_BASE_ADDR + 0x0);
			udelay(600000);
			val &=  ~(1 << 27);
			writel(val, GPIO6_BASE_ADDR + 0x0);
			udelay(600000);
...
out_poweroff:
	...
	/*Shutdown the device through WDOG*/
	mxc_request_iomux(MX50_PIN_WDOG, IOMUX_CONFIG_ALT1);
	val = readl(GPIO6_BASE_ADDR + 0x04);
	val |= (1<<28);
	writel(val, GPIO6_BASE_ADDR + 0x04);
	printf("Shutdown the device through WDOG\n");
	val = readl(GPIO6_BASE_ADDR + 0x0);
	val &=  ~(1 << 28);
	writel(val, GPIO6_BASE_ADDR + 0x0);
}
...
#define USBCMD 0x140
	...
	mxc_iomux_set_pad(MX50_PIN_KEY_ROW2, PAD_CTL_100K_PU |PAD_CTL_PUE_PULL |
											PAD_CTL_PKE_ENABLE | PAD_CTL_DRV_HIGH);
	mxc_iomux_set_pad(MX50_PIN_KEY_COL0, PAD_CTL_100K_PU |PAD_CTL_PUE_PULL |
											PAD_CTL_PKE_ENABLE | PAD_CTL_DRV_HIGH);

	value = readl(GPIO4_BASE_ADDR + 0x4);
	value &= ~(1<<5);
	writel(value, GPIO4_BASE_ADDR + 0x4);
	value = readl(GPIO4_BASE_ADDR + 0x4);
	value &= ~(1<<0);
	writel(value, GPIO4_BASE_ADDR + 0x4);
...
#ifdef CONFIG_MXC_EPDC
int need_auto_update(void)
{
	block_dev_desc_t *dev_desc=NULL;
	char *update_flag = "auto-update";
	struct mmc *mmc;
	char *devname = "mmc";
	int dev_num = 2;/*Read infomation from EMMC*/
	int part = 3;   /*Read file from EMMC 2:4*/
	long offset=0x70800000;
	int count = 10;
	int size = 0;
	...
	if (fat_register_device(dev_desc, part)!=0){
		printf ("\n** Unable to use %s %d:%d for fatload **\n",devname,dev_num,part);
		return 0;
	}

	size = file_fat_read (update_flag, (unsigned char *) offset, count);
	if(size==-1) {
		printf("\n** Unable to read \"%s\" from %s %d:%d **\n",update_flag,devname,dev_num,part);
		return 0;
	}
...
void check_for_onyx_updates(void)
{
	int i, j;
	char *a = NULL;
	unsigned int val;
	u8 *fb = (u8 *)lcd_base;
	char temp[UPDIMG_WIDTH];
	int auto_update_flag = 0;

	auto_update_flag = need_auto_update();

	val = readl(GPIO4_BASE_ADDR + 0x0);
	if ((((val>>5) & 0x1) == 0) || (((val >> 0) & 0x1) == 0) || (auto_update_flag == 1))
	{
		printf("Software update reqest detected,checking updates.auto_update=%d\n", auto_update_flag);
		setenv("bootargs", a);
#ifdef CONFIG_ONYX_ANDROID
		setenv("bootcmd_mmc", "run bootargs_base bootargs_video bootargs_mmc;mmc read 2 0x70800000 0x5000 0x2000;bootm");
		setenv("bootargs_mmc","setenv bootargs ${bootargs} console=ttymxc0,115200 androidboot.console=ttymxc0 root=initramfs init=/init");
#else
		setenv("bootcmd_mmc", "run bootargs_base bootargs_video bootargs_mmc;mmc read 2 0x70800000 0x3000 0x2000;bootm");
		setenv("bootargs_mmc","setenv bootargs ${bootargs} root=initramfs init=/init");
#endif
		
		if (panel_info.vl_col != 1200){
			for (i = 0; i<panel_info.vl_row; i++)
			{
				if (i >= UPDIMG_HEIGHT){
					memset(fb + i*panel_info.vl_col, 
								0xff, UPDIMG_WIDTH);
				}
				else {
					memcpy(fb + i*panel_info.vl_col,
								&upd_image[i][0], UPDIMG_WIDTH);
				}
			}
		}
		else{
			for (i = 0; i<panel_info.vl_row; i++){
				if (i >= UPDIMG_HEIGHT){
					memset((fb + (panel_info.vl_row - i)*panel_info.vl_col) - UPDIMG_WIDTH, 
								0xff, UPDIMG_WIDTH);
				}
				else {
					for (j = 0; j<UPDIMG_WIDTH; j++)
					{
						temp[j] = upd_image[i][UPDIMG_WIDTH - j];
					}
					memcpy ((fb + (panel_info.vl_row - i)*panel_info.vl_col) - UPDIMG_WIDTH, 
								&temp[0], UPDIMG_WIDTH);
				}
			}
		}
