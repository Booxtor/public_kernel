...
extern int setup_splash_img(void);
extern void check_for_onyx_updates(void);
...
static void epdc_init_settings(void)
{
	 ...
	 */
	reg_val =
		((panel_info.vscan_holdoff << EPDC_TCE_CTRL_VSCAN_HOLDOFF_OFFSET) &
	...
	/* EPDC_TCE_OE */
	reg_val =
		((panel_info.sdoed_width << EPDC_TCE_OE_SDOED_WIDTH_OFFSET) &
		EPDC_TCE_OE_SDOED_WIDTH_MASK)
	    | ((panel_info.sdoed_delay << EPDC_TCE_OE_SDOED_DLY_OFFSET) &
		EPDC_TCE_OE_SDOED_DLY_MASK)
	    | ((panel_info.sdoez_width << EPDC_TCE_OE_SDOEZ_WIDTH_OFFSET) &
		EPDC_TCE_OE_SDOEZ_WIDTH_MASK)
	    | ((panel_info.sdoez_delay << EPDC_TCE_OE_SDOEZ_DLY_OFFSET) &
	...
	/* EPDC_TCE_TIMING2 */
	reg_val =
	    ((panel_info.gdclk_hp_offs << EPDC_TCE_TIMING2_GDCLK_HP_OFFSET) &
		EPDC_TCE_TIMING2_GDCLK_HP_MASK)
	    | ((panel_info.gdsp_offs << EPDC_TCE_TIMING2_GDSP_OFFSET_OFFSET) &
		EPDC_TCE_TIMING2_GDSP_OFFSET_MASK);
	REG_WR(EPDC_BASE, EPDC_TCE_TIMING2, reg_val);

	/* EPDC_TCE_TIMING3 */
	reg_val =
	    ((panel_info.gdoe_offs << EPDC_TCE_TIMING3_GDOE_OFFSET_OFFSET) &
		EPDC_TCE_TIMING3_GDOE_OFFSET_MASK)
	    | ((panel_info.gdclk_offs << EPDC_TCE_TIMING3_GDCLK_OFFSET_OFFSET) &
...
static void draw_mode0(void)
{
	...
	printf("Mode0 update - Waiting for LUT to complete...%d*%d\n", panel_info.vl_col, panel_info.vl_row);

	for (i = 0; i < 60; i++) {
		if (!epdc_is_lut_active(0)) {
			puts("Mode0 init complete\n");
			return;
		}
		msleep(100);
	}

	puts("Mode0 init failed!\n");
...
void lcd_enable(void)
{
	...
	epdc_power_on();
	/* Load splash image to framebuffer */
	setup_splash_img();
	check_for_onyx_updates();
...
int do_flash (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	/* Draw data to display */
	draw_mode0();
    //check_for_onyx_updates_test();
	draw_splash_screen();
    return 0;
}

U_BOOT_CMD(flash, 2, 0, do_flash,
	"flash display",
	""
);
...
void lcd_ctrl_init(void *lcdbase)
{
	...
	struct waveform_data_header* wf_hdr = 
		(struct waveform_data_header*)CONFIG_WAVEFORM_BUF_ADDR;
	int hdr_size = ALIGN(sizeof(struct waveform_data_header) + wf_hdr->trc + 2, 4);
	int *p = (int *)(CONFIG_WAVEFORM_BUF_ADDR + hdr_size + 4);

	/* *p is the total header size */
	panel_info.epdc_data.waveform_buf_addr += *p;
