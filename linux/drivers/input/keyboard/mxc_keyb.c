...
#define TOUCHPANEL_DETECT_PIN_GPIO4_12    (3*32+12)

#define POWON3_STATUS_BIT_LSH	2
#define POWON3_STATUS_BIT_WID	1
...
struct keypad_priv {
	 ...
	 *power key status
	 */
	unsigned char  power_key_status;

	/*
	 *Touch Panel pen detect gpio pin interrupt.
	 */
	int pen_irq;
	 ...
	 *touch pen status.
	 */
	unsigned char touch_pen_status;

	/*
	 *usb detect status
	 */
	unsigned char usb_detect_status;
	/*!
	 * The base address
	 */
	void __iomem *base;
...
extern PMIC_STATUS pmic_read_reg(int reg, unsigned int *reg_value,
					  unsigned int reg_mask);
extern PMIC_STATUS pmic_write_reg(int reg, unsigned int reg_value,
					   unsigned int reg_mask);
...
static int mxc_kpp_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (device_may_wakeup(&pdev->dev)) {
		...
		enable_irq_wake(kpp_dev.pen_irq);

	} else {
		disable_irq(keypad->irq);
		disable_irq_wake(kpp_dev.pen_irq);
		key_pad_enabled = 0;
		clk_disable(kpp_clk);
...
irqreturn_t touchpen_detect_handler(int irq, void *dev_id)
{
	int value = 0;
	value = __gpio_get_value(TOUCHPANEL_DETECT_PIN_GPIO4_12);
	if (kpp_dev.touch_pen_status != value)
	{
		input_report_key(mxckbd_dev, KEY_F23, value);
	}

	kpp_dev.touch_pen_status = value;

	return IRQ_RETVAL(1);
}

static int register_on_touchpen_detect_handler(void)
{
	int ret = 0;

	ret = gpio_request(TOUCHPANEL_DETECT_PIN_GPIO4_12, "touch_pen_detect_pin");
	if (ret)
	{
		printk(KERN_ERR "Request GPIO %d fialed\n",TOUCHPANEL_DETECT_PIN_GPIO4_12);
		return ret;
	}
	gpio_direction_input(TOUCHPANEL_DETECT_PIN_GPIO4_12);
	kpp_dev.touch_pen_status =__gpio_get_value(TOUCHPANEL_DETECT_PIN_GPIO4_12);
	set_bit(KEY_F23, mxckbd_dev->keybit);
	if (kpp_dev.touch_pen_status)
	{
		input_report_key(mxckbd_dev, KEY_F23, kpp_dev.touch_pen_status);
	}

	kpp_dev.pen_irq = gpio_to_irq(TOUCHPANEL_DETECT_PIN_GPIO4_12);
	ret =request_irq(kpp_dev.pen_irq, touchpen_detect_handler,
			IRQF_TRIGGER_RISING|IRQF_TRIGGER_FALLING, "touchpen_detect_irq",0);
	if (ret)
	{
		printk(KERN_ERR "Failed to request irq for TOUCHPANEL_DETECT_PIN_GPIO4_12!\n");
		return ret;
	}

	return ret;
}
void usb_detect_callback(void *param)
{
	unsigned int value;

	pmic_read_reg(REG_INT_SENSE0, &value, BITFMASK(BIT_CHG_DETS));
	value = BITFEXT(value, BIT_CHG_DETS);
	if (kpp_dev.usb_detect_status != value)
	{
		input_report_key(mxckbd_dev, KEY_F21, value);
	}

	kpp_dev.usb_detect_status = value;
}

EXPORT_SYMBOL(usb_detect_callback);

static void register_usb_detect_handler(void)
{
	int ret;
	unsigned value;

	ret = pmic_read_reg(REG_INT_SENSE0, &value, BITFMASK(BIT_CHG_DETS));
	kpp_dev.usb_detect_status = BITFEXT(value, BIT_CHG_DETS);
	set_bit(KEY_F21,mxckbd_dev->keybit);
	if(kpp_dev.usb_detect_status)
	{
		input_report_key(mxckbd_dev, KEY_F21, kpp_dev.usb_detect_status);
	}
...
static void pmic_powerkey_event_callback(void *param)
{
	unsigned int value;

	pmic_read_reg(REG_INT_SENSE1, &value, BITFMASK(POWON3_STATUS_BIT));
	value = BITFEXT(value, POWON3_STATUS_BIT);

	if (kpp_dev.power_key_status == 1 && value == 1)
	{
		input_report_key(mxckbd_dev, KEY_POWER, 1);
		input_report_key(mxckbd_dev, KEY_POWER, 0);
	}

	if (kpp_dev.power_key_status != value)
	{
		input_report_key(mxckbd_dev, KEY_POWER, kpp_dev.power_key_status);
	}

	kpp_dev.power_key_status = value;
}

static void register_on_power_key_handler(void)
{
	pmic_event_callback_t powon_event_callback;
	unsigned int value;

	pmic_read_reg(REG_INT_SENSE1, &value, BITFMASK(POWON3_STATUS_BIT));
	kpp_dev.power_key_status = BITFEXT(value, POWON3_STATUS_BIT);
	
	set_bit(KEY_POWER, mxckbd_dev->keybit);

	powon_event_callback.func = pmic_powerkey_event_callback;
	powon_event_callback.param= NULL;
	pmic_event_subscribe(EVENT_PWRON3I, powon_event_callback);
}
...
static int mxc_kpp_probe(struct platform_device *pdev)
{
	...
	register_on_power_key_handler();
	register_usb_detect_handler();
	register_on_touchpen_detect_handler();
