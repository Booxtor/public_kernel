...
//extern void rtc_interrupt_callback(void *param);
...
atomic_t rtc_state = ATOMIC_INIT(0);

static ssize_t rtc_int_state_show(struct device *dev,
						struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", atomic_read(&rtc_state));
}

static DEVICE_ATTR(rtc_int_state, 0644, rtc_int_state_show, NULL);

static void mxc_rtc_alarm_int(void *data)
{
	struct rtc_drv_data *pdata = data;

//	rtc_interrupt_callback(data);
	atomic_set(&rtc_state, 1);
...
static int mxc_rtc_probe(struct platform_device *pdev)
{
	...
	retval = sysfs_create_file(&pdev->dev.kobj, &dev_attr_rtc_int_state.attr);
	if (retval) {
		printk(KERN_ERR"Battery: Unable to register sysdev for USB state");
		goto  failed;
	}
...
#ifdef CONFIG_PM
static int mxc_rtc_suspend(struct platform_device *dev, pm_message_t state)
{
	atomic_set(&rtc_state, 0);
	return 0;
}

static int mxc_rtc_resume(struct platform_device *dev)
{
	return 0;
}
#endif
