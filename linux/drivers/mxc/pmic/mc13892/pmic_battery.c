...
#define BIT_BPSNS_LSH    16
#define BIT_BPSNS_WID    2
...
#define BIT_CHG_CCCVS_LSH 10
#define BIT_CHG_CCCVS_WID 1
...
#define  BP_DETECTION_THROSHOLDS_3V0     0
#define  BP_DETECTION_THROSHOLDS_3V1     1
#define  BP_DETECTION_THROSHOLDS_3V3     2
#define  BP_DETECTION_THROSHOLDS_3V4     3
...
pmic_event_callback_t cccv_event_callback;
pmic_event_callback_t lobatl_event_callback;
extern int kobject_uevent(struct kobject *kobj, enum kobject_action action);
extern PMIC_STATUS	mc13892_adc_temp_convert(t_channel channel, unsigned int *result);
extern void usb_detect_callback(void *param);
extern atomic_t screensplash_flag;
...
static int pmic_restart_charging(void)
{
	...
	pmic_set_chg_misc(CYCLING_DIS, 0);
...
static int mc13892_charger_cycling(struct mc13892_dev_info *di)
{
	int ret;
	int online;
	unsigned int value;
	...
	ret = pmic_read_reg(REG_INT_SENSE0,&value, BITFMASK(BIT_CHG_CURRS));
	if (ret == 0) {
		charging = BITFEXT(value, BIT_CHG_CURRS);
	}
			
	ret = pmic_read_reg(REG_INT_SENSE0, &value, BITFMASK(BIT_CHG_DETS));
	if (ret == 0) {
		online = BITFEXT(value, BIT_CHG_DETS);
		if (online && charging != 1)
	    {   
	    	...
	    	printk(KERN_INFO"%s:charger cycling start!\n",__func__);
...
static void charger_online_event_callback(void *para)
{
	...
	usb_detect_callback(para);
}

static void charge_cycling_event_callback(void *para)
{
	struct mc13892_dev_info *di = (struct mc13892_dev_info *) para;
	mc13892_charger_cycling(di);
}  

static void pmic_lowbattery_event_callback(void *para)
{
	struct mc13892_dev_info *di = (struct mc13892_dev_info *) para;
	t_channel channel = BATTERY_VOLTAGE;
	unsigned int result = 0, voltage_mV = 0;

	if ((0 == di->charger_online) && (atomic_read(&screensplash_flag) == 0))
	{
		mc13892_adc_temp_convert(channel,&result);
		voltage_mV = (result*BAT_VOLTAGE_UNIT_UV)/1000;
		if (voltage_mV < 3350)
		{
			kobject_uevent(&(di->dev->kobj), KOBJ_CHANGE);
			di->lowbattery_state = 1;
			return ;
		}
	}

	di->lowbattery_state = 0;

	return ;
}

static int mc13892_battery_property_is_writeable(struct power_supply *psy,enum power_supply_property psp)
{
	switch (psp) {
...
static int mc13892_battery_set_property(struct power_supply *psy,
				       enum power_supply_property psp,
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		val->intval = di->lowbattery_state;
		break;
	default:
		return -EINVAL;
	}
				...
				struct device_attribute *attr, char *buf)
{
	...
	ret = pmic_read_reg(REG_INT_SENSE0, &value, BITFMASK(BIT_CHG_DETS));
	if (ret == 0) {
		usb_state = BITFEXT(value, BIT_CHG_DETS);
	}
...
static DEVICE_ATTR(usb_state, 0644, usb_state_show, NULL);
...
static int pmic_set_bp_thresholds( unsigned short  thred )
{
	unsigned int mask;
	unsigned int value;

	value = BITFVAL(BIT_BPSNS, thred);
	mask = BITFMASK(BIT_BPSNS);

	return pmic_write_reg(REG_POWER_CTL0, value, mask);
}
...
static int pmic_battery_probe(struct platform_device *pdev)
{
	...
	di->charger.type = POWER_SUPPLY_TYPE_USB;
	...
	di->bat.set_property = mc13892_battery_set_property;
	di->bat.property_is_writeable = mc13892_battery_property_is_writeable;
	...
	cccv_event_callback.func = charge_cycling_event_callback;
	cccv_event_callback.param = (void *)di;
	CHECK_ERROR(pmic_event_subscribe(EVENT_CCCVI, cccv_event_callback));	

	/*Set low battery detection throsholds */
	pmic_set_bp_thresholds(BP_DETECTION_THROSHOLDS_3V3);

	lobatl_event_callback.func = pmic_lowbattery_event_callback;
	lobatl_event_callback.param= (void *)di;
	CHECK_ERROR(pmic_event_subscribe(EVENT_LOBATHI, lobatl_event_callback));
	...
	retval = sysfs_create_file(&pdev->dev.kobj, &dev_attr_usb_state.attr);
	if (retval) {
		printk(KERN_ERR
		       "Battery: Unable to register sysdev for USB state");
		goto workqueue_failed;
	}
	chg_wa_is_active = 1;
	chg_wa_timer = 0;
