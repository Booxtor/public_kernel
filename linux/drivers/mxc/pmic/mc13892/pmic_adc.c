...
PMIC_STATUS mc13892_adc_temp_convert(t_channel channel, unsigned int *result)
{
	unsigned int adc_0_reg, adc_1_reg;
	int ret = 0;
	int channel_0 = channel;
	int channel_1 = 0;

	adc_0_reg = 0;
	adc_0_reg = 0x0 & 0x000003F;
	...
	adc_1_reg |= ASC_ADC;
	ret = pmic_write_reg(REG_ADC1, adc_1_reg,ADC_SGL_CH |
			ADC_CH_0_MASK |ADC_CH_1_MASK |
			ADC_NO_ADTRIG | ADC_EN | ASC_ADC);
	if (ret != PMIC_SUCCESS) {
		pr_debug("pmic_write_reg");
		return ret;
	}
	...
	ret = pmic_read_reg(REG_ADC2, result, PMIC_ALL_BITS);
	if (ret != PMIC_SUCCESS) {
		pr_debug("pmic_write_reg");
		return ret;
	}

	*result = ((*result) & ADD1_RESULT_MASK) >> 2;

	return ret;
}
EXPORT_SYMBOL(mc13892_adc_temp_convert);
