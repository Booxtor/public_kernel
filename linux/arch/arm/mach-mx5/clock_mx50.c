...
int __init mx50_clocks_init(unsigned long ckil, unsigned long osc, unsigned long ckih1)
{
	...
	/* Change the SSI_EXT1_CLK to be sourced from PLL3_SW_CLK */
	clk_set_parent(&ssi_ext1_clk, &pll3_sw_clk);
	...
	clk_set_parent(&gpmi_nfc_clk[0], &pll1_main_clk);
	clk_set_rate(&gpmi_nfc_clk[0], 24000000);
